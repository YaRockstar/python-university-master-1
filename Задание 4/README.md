...
Задание 4/
├── main.py                    # Точка входа
├── cli.py                     # Консольный интерфейс для работы с системой
│
...
Для интерактивной работы с системой используется файл `cli.py`:
...
- добавлять транспортные средства различных типов;
- добавлять водителей;
- назначать водителя на транспортное средство;
- просматривать список всех ТС и водителей;
- выполнять поиск по модели;
- анализировать вместимость по типам;
- сохранять данные в JSON.

Интерактивный интерфейс полностью использует бизнес-логику проекта и проверку прав доступа.
...
8. Позволяет работать через интерактивный CLI (`cli.py`)
...
Для интерактивного режима используйте `cli.py`.
